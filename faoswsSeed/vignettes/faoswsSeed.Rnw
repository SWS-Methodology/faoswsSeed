...
for(file in dir("Documents/Github/sws_imputation/faoswsImputation/R/", full.names = TRUE))
    source(file)
for(file in dir("Documents/Github/sws_seed/faoswsSeed/R/", full.names = TRUE))
    source(file)
library(data.table)
library(ggplot2)
@ 
...
ratio within each country individually:
...
data[geographicAreaM49 == 100 & measuredItemCPC == "0111",
     .(time = timePointYears, areaSown = Value_measuredElement_5212,
       areaHarvested = Value_measuredElement_5312, Value_areaSownRatio)]
...
countrySpecificData = data.table(
    geographicAreaM49 = c("100", "348", "400", "804", "894"),
    measuredItemCPC = "0111",
    Value_seedRate = c(222, 213, 115, 241, 95),
    flagObservationStatus_seedRate = c("E", "E", "", "E", "E"))
...
simpler to just create the part of the table we use at the time of the writing,
and this is what is done here.  The fillCountrySpecificSeedRate function
...
If the specific\_seed\_rate table had not provided a value for this
country/commodity pair, then the first code block in this section would have
set Value\_seedRate to NA.  Then, this second code block would have updated the
